...
- (void)searchStationWithGetupPoi:(AMapPOI *)getupPoi getdownPoi:(AMapPOI *)getdownPoi type:(TYWJCommuteControllerType)type {
    self.getupPoi = getupPoi;
    self.getdownPoi = getdownPoi;
    self.searchResult = nil;
    self.searchResult = [NSMutableArray array];
    if (![self p_getAllRouteStationsWithType:type]) {
        [self p_loadRouteListData];
    }else {
        [self p_doSearchStataion];
    }
}

- (NSArray *)p_getAllRouteStationsWithType:(TYWJCommuteControllerType)type  {
    if (self.selectedCity.cityID.intValue == self.selectedCityID && self.allRouteStations.count && self.selectedType == type) {
        return self.allRouteStations;
    }
    self.selectedType = type;
    self.selectedCityID = self.selectedCity.cityID.intValue;
    return nil;
}
...
/**
 加载线路表数据
 */
- (void)p_loadRouteListData {
    WeakSelf;
    NSString *category = nil;
    switch (self.selectedType) {
        case TYWJCommuteControllerTypeCommute:
        {
            category = @"通勤";
        }
            break;
        case TYWJCommuteControllerTypeCampus:
        {
            category = @"校园";
        }
            break;
        case TYWJCommuteControllerTypePlane:
        {
            category = @"飞机场";
        }
            break;
        case TYWJCommuteControllerTypeTrain:
        {
            category = @"火车站";
        }
            break;
    ...
    [TYWJSoapTool SOAPDataWithoutLoadingWithSoapBody:bodyStr success:^(id responseObject) {
        ...
        if (responseObject) {
            ...
            if ([data isKindOfClass: [NSArray class]]) {
                ...
                if (dataArr.count) {
                    [weakSelf p_getDetailRouteWithDataArr:dataArr];
                }
            }else if([data isKindOfClass: [NSDictionary class]]){
                TYWJRouteList *route = [TYWJRouteList mj_objectWithKeyValues:data];
                if (route) {
                    NSArray *dataArr = @[route];
                    [weakSelf p_getDetailRouteWithDataArr:dataArr];
                }
            }else {
                ZLLog(@"++++请求的routelist数据为空++++");
...
- (void)p_getDetailRouteWithDataArr:(NSArray *)dataArr {
    self.routeList = dataArr;
    self.allRouteStations = [NSMutableArray array];
    self.counter = 0;
    self.allRouteCount = dataArr.count;
    for (TYWJRouteList *route in dataArr) {
        [self p_getDetailRouteWithRouteNum:route.routeListInfo];
    }
...
- (void)p_getDetailRouteWithRouteNum:(TYWJRouteListInfo *)routeInfo {
    ...
    [TYWJSoapTool SOAPDataWithSoapBody:soapBodyStr success:^(id responseObject) {
        ...
        if (responseObject) {
            weakSelf.counter++;
            ZLFuncLog;
            id data = responseObject[0][@"NS1:xianlubiaozibiaoResponse"][@"xianlubiaozibiaoList"][@"xianlubiaozibiao"];
            if (data) {
                NSArray *dataArr = [TYWJSubRouteList mj_objectArrayWithKeyValuesArray:data];
                if (dataArr) {
                    NSDictionary *dataDic = @{kCommonToolSearchDataKey : dataArr,kCommonToolSearchRouteInfoKey : routeInfo};
                    [weakSelf.allRouteStations addObject:dataDic];
                }
            }else {
                ZLLog(@"++++请求的routelist数据为空++++");
        ...
        if (weakSelf.counter == weakSelf.allRouteCount) {
            //所有数据请求完毕
            [self p_doSearchStataion];
        }
    } failure:^(NSError *error) {
...
- (void)p_doSearchStataion {
    NSInteger cc = self.routeList.count;
    NSArray *tmpAllRoutes = [self.allRouteStations copy];
    [self.allRouteStations removeAllObjects];
    for (NSInteger i = 0; i < cc; i++) {
        TYWJRouteList *route = self.routeList[i];
        
        for (NSDictionary *subRoute in tmpAllRoutes) {
            TYWJSubRouteListInfo *srInfo = subRoute[kCommonToolSearchRouteInfoKey];
            if (route.routeListInfo.routeNum.integerValue == srInfo.routeNum.integerValue) {
                [self.allRouteStations addObject:subRoute];
                break;
            }
        }
    }
...
//    MAMapPoint getdownPoint = MAMapPointForCoordinate(CLLocationCoordinate2DMake(self.getdownPoi.location.latitude,self.getdownPoi.location.longitude));
    for (NSDictionary *dataDic in self.allRouteStations) {
        ...
        TYWJSearchReult *result = [[TYWJSearchReult alloc] init];
        result.startStation = [(TYWJSubRouteList *)[lists firstObject] routeListInfo];
        result.desStation = [(TYWJSubRouteList *)[lists lastObject] routeListInfo];
        result.getupDistance = dis + 1;
        result.getdownDistance = dis + 1;
