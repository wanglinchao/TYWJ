...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _isNeedShowRetangle = NO;
        _photoframeLineW = 6;
        _notRecoginitonArea = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
        _scanRetangleRect = CGRectMake(60, 150, frame.size.width - 2 * 60, frame.size.width - 2 * 60);
...
//        CGContextStrokePath(context);
    ...
    CGContextSetStrokeColorWithColor(context, [UIColor colorWithHexString:@"FED302"].CGColor);
...
- (UIButton *)flashBtn{
    if (_flashBtn == nil) {
        _flashBtn = [[UIButton alloc] initWithFrame:CGRectMake((self.frame.size.width - 40)/2.0, self.scanRetangleRect.origin.y + self.scanRetangleRect.size.height + 10, 40, 40)];
